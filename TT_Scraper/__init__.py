...
class TT_Scraper(HTML_Scraper):
    def __init__(self, wait_time=0.35, output_files_fp="data/", browser_name=None):
        ...
        if browser_name:
            self.cookies = getattr(browser_cookie3, browser_name)(
    ...
    def scrape_list(self, ids: list = None, scrape_content: bool = True, batch_size: int = 0, clear_console=False, total_videos=0, already_scraped_count=0, total_errors=0):
        ...
        for self.iterations, id in enumerate(ids, start=1):
            ...
            metadata_package, content_binary = self.scrape(
                id=id, scrape_content=scrape_content, download_metadata=False, download_content=False)
    ...
    def scrape(self, id, scrape_content: bool = False, download_metadata=True, download_content=True):
        ...
        try:
            # scraping html data
            requested_data = self.request_and_retain_cookies(
                f"https://www.tiktok.com/@tiktok/video/{id}")
            html_parser = BeautifulSoup(requested_data.text, "html.parser")
            rehydration_data = html_parser.find(
            ...
            if rehydration_data_json:
                try:
                    interesting_elements = rehydration_data_json["__DEFAULT_SCOPE__"][
            ...
            if scrape_content:
                try:
                    binary_video = self._scrape_video(
                        metadata=rehydration_data_json)
                    metadata_package["file_metadata"]["is_slide"] = False
                except VideoIsPicture:
                    binaries_slides, picture_formats, binary_slides_audio = self._scrape_picture(
                        metadata=rehydration_data_json)
        ...
        # handling exceptions
            ...
            metadata_package = self._exception_handler(
                id, error_code, "NoDataFromURL")
            content_binary = None
        except ItemInfoError:
            error_code = "I"
            self.log.warning(
                "The scraped HTML code could not provide the searched metadata elements")
            metadata_package = self._exception_handler(
                id, error_code, "ItemInfoError")
            content_binary = None
        except VideoNotFoundError:
            error_code = "V"
            self.log.warning("No data from URL")
            metadata_package = self._exception_handler(
                id, error_code, "VideoNotFoundError")
            content_binary = None
        except OtherError:
            error_code = "O"
            metadata_package = self._exception_handler(
                id, error_code, "OtherError")
            content_binary = None
        except RetryLaterError:
            # retry
            self.repeated_error += 1
            self.log.warning(
                "-> retrying video due to error in package download...")
        ...
        if binary_video:
            ...
            content_binary = {
                "type": "slide", "slide_pictures": binaries_slides, "slide_audio": binary_slides_audio}
        else:
            content_binary = None
        ...
        if download_metadata and download_content:
            ...
            metadata_package["content_binary"] = content_binary
            self._download_data(
                metadata_batch=[metadata_package], download_metadata=False)
            return metadata_package
        elif download_metadata and not download_content:
            metadata_package["content_binary"] = content_binary
            self._download_data(
                metadata_batch=[metadata_package], download_content=False)
    ...
    def scrape_user(self, username, download_metadata=True):
        ...
        if download_metadata:
            self.write_metadata_package(
                filepath=filepath, metadata_package=user_data)
        else:
            return user_data
